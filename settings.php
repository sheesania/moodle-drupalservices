...
if($config->cookiedomain) {
  ...
  if ($loggedin_user = $apiObj->Connect()) {
    if ($loggedin_user->user->uid !== false) {
      ...
      $fulluser = array(
          'name', 'mail', 'timezone', 'language', 'uid',
          'field_user_city', 'field_first_name', 'field_last_name', 'field_company_name',
          'field_phone', 'field_greeting', 'field_source', 'field_date_modified',
          'field_admin_notes', 'field_country', 'field_user_position', 'field_existing_course_his',
          'field_legacy_mailing_street_addr', 'field_legacy_mailing_state', 'field_legacy_mailing_zip'
      );

    $fulluser_keys = array_combine($fulluser, $fulluser);
  } else {
