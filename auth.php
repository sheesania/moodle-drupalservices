...
class auth_plugin_drupalservices extends auth_plugin_base
{
    ...
    function loginpage_hook()
    {
        ...
        if ($drupalsession == null) {
            ...
            if (isset($SESSION->wantsurl) and (strpos($SESSION->wantsurl, $CFG->wwwroot) == 0)) {
                ...
                $special="";
                if (!empty($this->config->use_special_goto)) {
                  $special = "moodle_";
                }
   ...
   */
    ...
    function create_update_user($drupal_user)
    {
        ...
        if($uid == '11') {
          $user = $DB->get_record('user', array('username' => 'keith')); // rehab added for temp keith bypass
        } else {
          $user = $DB->get_record('user', array('idnumber' => $uid, 'mnethostid' => $CFG->mnet_localhost_id));  
        }

        if (empty($user)) {
          // rehab add to check if we find match on email
          $dmail = $drupal_user->mail;
          $user = $DB->get_record('user', array('email' => $dmail));
          if (empty($user)) {
        ...
        if (empty($user->id)){
            ...
            $user_original = $user; // rehab added for below
            $user = $DB->get_record('user', array('username' => $user->username, 'mnethostid' => $CFG->mnet_localhost_id));
            if (!$user) { // rehab added to insert user with '-d' appended to prevent duplicate username
              $user->username = $drupal_user->name.'-d';
              $uid = $DB->insert_record('user', $user);
              $user = $DB->get_record('user', array('username' => $user->username, 'mnethostid' => $CFG->mnet_localhost_id));
            }
            if (!$user) {
